...
class PurchaseCreateBillWizard(models.TransientModel):
    ...
    ], string='Billing Option', default='ready_only', required=True)
            ...
            if purchase_id:
                ...
                for line in purchase_order.order_line:
                    ...
                    if line.product_id.purchase_method == 'purchase':
                        ...
                        if qty_to_bill > 0:
                            ready_lines.append(f"• {line.product_id.name} - Qty: {qty_to_bill} (Can immediately billed)")
                        ...
                        if qty_ready_to_bill > 0:
                            ready_lines.append(f"• {line.product_id.name} - Qty: {qty_ready_to_bill} (Already received)")
                        
                        # Check if there's quantity still waiting to be received
                        qty_waiting = qty_ordered - qty_received
                        if qty_waiting > 0:
                            waiting_lines.append(f"• {line.product_id.name} - Qty: {qty_waiting} (Need to receive)")
    ...
    def action_create_bill(self):
        ...
        if self.bill_option == 'wait_all':
            return {
                ...
                'params': {
                    'message': _('Please receive all pending items first, then try creating the bill again.'),
                    'type': 'warning',
                    'sticky': False,
                }
            }
